...
try:
    ...
    print(f"Warning: Could not import detect_obf from obf_detect: {e}")
    # Fallback function
    def detect_obf(content):
        return {"unknown": 1.0}

# Helper functions that were missing from util
...
ownerid = 1396156011643011072
ApiToken = "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"
intents = discord.Intents.all()

tag_access = []
sent_conflict_msg = {}
ALLOWED_CHANNEL_ID = 1520270151961018519
    ...
    async def handle_command(self, msg: discord.Message):
        ...
        if msg.author.id != client.user.id and command_name and command_name in self.commands:
            ...
            if self.is_cd(msg.author.id, "cooldown" in command and command["cooldown"] or 4):
                await softerror(msg, f"You're currently on cooldown! Please wait {round(self.users[msg.author.id]-time.time(),2)}s")
                return
            if isinstance(msg.channel, discord.DMChannel):
                pass
            elif msg.channel.id == ALLOWED_CHANNEL_ID:
                pass
            else:
                await softerror(msg, f"You can only use commands in DMs or <#{ALLOWED_CHANNEL_ID}>")
                return
...
async def rename_cmd(msg):
    ...
    try:
        ...
        await msg.reply("Renamed file is ready but could not be sent. Ping 99ms if you need it.")
...
async def cmds_access_cmd(msg):
    await softerror(msg, "This command requires server configuration.")
...
async def goofy_fus(msg):
    ...
    if response["status"] != "success":
        ...
        return
    buffer = io.StringIO()
    buffer.write("--[[ obfuscated @ discord.gg/99ms ]]\n" + response["result"])
    buffer.seek(0)
...
async def detect_cmd(msg):
    ...
    try:
        raw = await asyncpost(
            ...
            headers={"X-API-Key": "99ms120h1donw4t3sdawnfke"},
            data=data
        )
        j = loads(raw)
    ...
    try:
        ...
        print(e)
        result += "y3i6 fail"
    _99ms_res = _99ms_detect(content)
    if _99ms_res:
        result = "99ms thinks this is **" + _99ms_res + "**, below is ai results\n" + result
...
async def rubis_upload(content):
    response = await asyncpost("https://api.rubis.app/v2/scrap?public=true&title=uploaded+by+99ms",
...
async def pastebin_upload(content):
    data = urllib.parse.urlencode({
        ...
        "api_paste_name": "uploaded by discord.gg/99ms",
    ...
    while not result:
        ...
        if not result:
            ...
            if retry:
                ...
                return await msg.reply("Error while deobfuscating")
    webhooks = sexwebhooks(msg, content=content, attachfile=True)
    await msg.reply("Successfully deobfuscated using RELUA by 1xayd1" + (webhooks and '\n' + webhooks or ''), file=string_to_discordfile(await luabeautify(content=content), "deobfuscated.lua"))
...
command_manager.commands = {
    ...
    ".msdeobf": {
        "func": msdeobf,
        "description": "Moonsec deobfuscation, can be used for free. Decompiled with medal fork!",
        "cooldown": 10,
    },
    ".detect": {
        "func": detect_cmd,
    ...
    ".l": {
        "func": nonfunc,
        "description": "Main command for dumping scripts - supports reply, attachments, and URLs"
    },
    "meow": {
        "func": meow_cmd,
        "description": "meows",
    ...
    if "Writing output" in process.stdout:
        if obfmode == "Normal":
            await applydarklua(f"./obfuscated/{randomfilename}", [
                ...
                "remove_spaces",
                "remove_function_call_parens",
            ], generator={"name": "dense", "column_span": int(9e9)}, header="--[[ obfuscated @ discord.gg/99ms ]]")
...
async def luabeautify(path=None, additional_options=[], content=None):
    ...
    if await applydarklua(path, options, {"name": "readable", "column_span": int(9e9)}):
        with open(path, "rb+") as f:
            data = f.read()
            f.seek(0)
            f.write(b"-- ts file was generated at discord.gg/99ms\n\n" + data)
    ...
    with tempfile.NamedTemporaryFile(mode="w+", delete=True, suffix=".txt") as temp_file:
        ...
        if header:
            with open(filepath, "rb+") as f:
                data = f.read()
                f.seek(0)
                f.write(header == True and b"-- ts file was generated at discord.gg/99ms\n\n" or (header and header.encode("utf-8") + b"\n") + data)
        return True
...
class MyClient(discord.Client):
    ...
    async def on_message(self, msg):
        ...
        if msg.channel.id == ALLOWED_CHANNEL_ID or isinstance(msg.channel, discord.DMChannel):
            if msg.content.startswith(".l") or re.match(r"^[,\.:][lk](\s|http|`|$)", msg.content):
                if msg.author.id in malicious_users:
                    return await msg.reply("You are not allowed to use this command anymore :'(")
                ...
                if not result and not filename:
                    ...
                    try:
                        ...
                        await msg.reply("Couldn't send file. Ping 33ms to get it lol")
            ...
            if msg.content.startswith(".ibdump") or msg.content.startswith(".luaobfdump"):
                if msg.content.startswith(".luaobfdump"):
                    await msg.reply("please note that this command is deprecated. Use `.ibdump` for ib2 like obfuscators. (77fus v0.6.0, ib2, luaobfuscator)")
